...
## Dritter Bugfix: RAM-Erschöpfung bei zwei gleichzeitigen WebIF-Clients — 2026-08-19

Nutzer-Beobachtung nach den ersten beiden Fixes: das Unerreichbar-Problem tritt scheinbar auf, wenn zusätzlich zum PC ein Handy die WebIF-Seite öffnet. Der zuvor als "zu alt" erkannte `diagnostics/lastError`-Zeitstempel (WLAN-Verlust) korrelierte nicht mit dem aktuellen Vorfall — legte die WLAN-Reconnect-These als Hauptursache auf Eis, RAM-Live-Check lieferte den eigentlichen Befund.

| # | Prüfpunkt | Test (was/wie) | Ergebnis | Bewertung |
|---|---|---|---|---|
| 1 | RAM-Live-Snapshot | MQTT-Snapshot (`diagnostics/ram`, `main/info/uptime`) am laufenden Gerät (Build 00027) | `ram = 97% (240/247 KB)` bei `uptime = 333s` — Erschöpfung innerhalb von nur ~5,5 Minuten, kein langsames Multi-Tage-Leck | ✅ (kritischer Befund) |
| 2 | Root-Cause-These | Code-Review `WebIF.cpp`: `serveStatic()` liefert alle Dateien mit `Cache-Control: no-cache, no-store, must-revalidate` aus (Dev-Provisorium seit 2026-08-17) | `no-store` verhindert jegliches Browser-Caching — jede Anfrage von jedem Gerät lädt alle Dateien komplett neu, inkl. `mqtt.min.js` (369 KB) und `alpine.min.js` (47 KB); zwei gleichzeitige Geräte verdoppeln die gleichzeitige Speicherlast auf einem ESP32 mit 512 KB RAM | ✅ (These, siehe Fix) |
| 3 | Library-Fähigkeiten geprüft | `.pio/libdeps/.../ESPAsyncWebServer/src/WebHandlers.cpp` durchsucht | `AsyncStaticWebHandler` hat bereits eingebaute ETag/Last-Modified-Logik inkl. automatischer `304 Not Modified`-Antwort bei passendem `If-None-Match` — nur durch `no-store` bislang nie genutzt | ✅ |
| 4 | Fix build + Flash | `Cache-Control` auf reines `"no-cache"` reduziert (`no-store`/`must-revalidate` entfernt), `pio run --target upload` | RAM 62,6% / Flash 45,1% (Build-Größe unverändert zum Vortest), Boot verifiziert: `availability=online`, Build 00030, `ram=75% (187/247 KB)` bei `uptime=3` — normale Baseline | ✅ |
| 5 | Zwei-Client-Test (PC + Handy) mit RAM-Verlauf | Nutzer öffnet WebIF gleichzeitig auf PC und Handy, `diagnostics/ram` beobachtet; zusätzlich per MQTT-Snapshot gegengeprüft (`uptime=185`) | `ram = 75% (187/247 KB)` — identisch zur Boot-Baseline, kein Anstieg trotz zwei gleichzeitiger Clients (vorher: 97% nach 333s mit derselben Konstellation) | ✅ Fix bestätigt |
