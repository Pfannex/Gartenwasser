...
- Flash-Checkpoint: 41,5 % → 43,2 % (≈+50 KB durch die neuen Bibliotheken), deutlich unter der vorab geschaetzten Groessenordnung. Seite laedt ueber die Geraete-IP im Browser (`curl`-verifiziert: HTTP 200 fuer `/`, `/index.html`, `/style.css`, korrekter Inhalt).
- Details siehe `docs/spec/16-webif-fundament.md`, `docs/testing.md`.

## Offene Punkte / nächste Schritte

- **`uploadfs`/`mklittlefs`-Formatinkompatibilität richtig lösen** — vor Phase 17, da der aktuelle PROGMEM-Workaround (Web-Dateien als C++-Strings in der Firmware eingebettet) nicht auf die deutlich größeren Dateien (Alpine.js, `mqtt.js`) skaliert.
- **Phase 17 (Web-Interface: Status-Dashboard, read-only)** — nächster Arbeitsschritt nach der `uploadfs`-Lösung: MQTT-over-WebSocket-Client (Alpine.js + `mqtt.js`) im Browser, testet den Lesepfad der in Phase 16 entschiedenen Architektur B. Voraussetzung außerhalb der Firmware: WebSocket-Listener auf dem Mosquitto-Broker einrichten (noch nicht erledigt).
