...
### Nachtrag (2026-08-18): Backend-Logging-Review, PUB/SUB jetzt im Live-Log, PubSubClient-Reentrancy-Bug behoben

Auf Nutzerwunsch jede `.cpp`-Datei einzeln auf Backend-Logging-Lücken durchgesehen (nicht die Frontend-Bedienung). Ergebnisse mit Bezug zu dieser Seite:

- `WebManager.cpp`: `onNotFound` loggt jetzt `DEBUG/WEB` mit der angefragten URL (kaputte/veraltete Links im Web-Interface aufspürbar).
- `ValveController.cpp`: `setAuto()`/`setAlias()` loggen jetzt `INFO/VALVE`.
- **Kurswechsel**: `PUB`/`SUB` waren bisher an der Quelle (`Logger::log()`) ausgeschlossen — die dort angenommene Rückkopplungsgefahr (weitergeleitete Zeile erzeugt neue PUB-Zeile, die wieder weitergeleitet wird) erwies sich auf Nachfrage des Nutzers als Denkfehler: `onLoggerLine()` nutzt ohnehin rohes `mqttClient.publish()`, nie die geloggte `publishAndLog()`. Freigeschaltet, mit gezielter Filterung nur der zwei sekündlich publizierenden Topics `main/remainingTotal`/`V{n}/time/remaining` (Rauschen ohne Zusatzinfo während einer laufenden Automatik).
- **Dabei aufgedeckter Bug**: `PubSubClient` teilt sich einen internen Puffer für ein-/ausgehende Pakete; das an `handleMqttMessage()` übergebene `topic` zeigt vermutlich direkt hinein. Ein synchrones `mqttClient.publish()` aus dem Empfangs-Callback heraus (hier: das Live-Log-Publish der gerade erzeugten `SUB`-Zeile) überschrieb diesen Puffer noch während der Verarbeitung derselben Nachricht — reproduziert als sporadische `MQTT ERROR Unbekanntes Topic: '<Datenmüll>'`-Zeile direkt nach jeder `SUB`-Zeile. **Fix**: Live-Log-Publishes laufen jetzt nur noch über einen Puffer (`pendingLogCount`/`deferredReplayRequested`), das eigentliche `mqttClient.publish()` passiert ausschließlich in `MqttManager::flushPendingLogLines()`, aufgerufen aus `MqttManager::loop()` direkt nach `mqttClient.loop()` — außerhalb jedes Empfangs-Callback-Kontexts.
- Auf Hardware verifiziert (`test_livelog_pubsub.py`, 108 Live-Log-Zeilen über Auto-Toggle/Programmwahl/Automatik-Start-Stopp/404): keine `Unbekanntes Topic`-Fehlzeilen mehr, `PUB`/`SUB` korrekt vertreten, die zwei Rausch-Topics weiterhin gefiltert, Kommandos weiterhin zuverlässig zugestellt. `data/log.js` `LOG_TYPES` um `PUB`/`SUB` ergänzt.
