...
- **Umzug auf eigene Seite** `data/log.html`/`data/log.js` (eigener Nav-Tab „Log", damit ist die Hauptnavigation aller fünf Seiten komplett aktiv) — schlanke eigene MQTT-Verbindung, abonniert nur `diagnostic/livelog` + `availability` statt des kompletten Wildcard-Topics.
