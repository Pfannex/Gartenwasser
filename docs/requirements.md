...
**`schedule`** (`main/schedule/state`, Phase 15, Design-Entwurf abgestimmt, Umsetzung offen): Array von Zeitplan-Einträgen (Trigger-Typ `daily`/`weekly`/`once`, Programm-Referenz **per Name** statt Array-Index, um Drift bei `main/programs/set`-Umsortierungen zu vermeiden — analog zum `shortcut`-Feld der Programme). Globaler Ein/Aus-Schalter (`enabled` auf oberster Ebene + Convenience-Topic `main/schedule/cmd`, analog `main/program/cmd`). Verpasste Trigger (Reboot im Startfenster) verfallen bewusst, statt nachgeholt zu werden. Details siehe `docs/spec/15-wochenplan.md`.
