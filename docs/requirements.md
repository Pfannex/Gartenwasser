...
Code-Struktur-Dokumentation ergänzt: Doxygen + Graphviz eingerichtet (2026-08-27): reines Tooling, keine funktionale Änderung. Erzeugt aus dem bestehenden Quelltext (`src/`, `include/`) automatisch Klassendiagramme, Aufruf-/Aufrufer-Graphen und Include-Abhängigkeitsgraphen — ohne dass eigene Doc-Kommentare nötig wären (`EXTRACT_ALL`). Konfiguration liegt versioniert unter `tools/doxygen/Doxyfile`, generierte Ausgabe (`tools/doxygen/output/`) bewusst nicht versioniert (jederzeit neu erzeugbar). Die XML-Ausgabe ist zusätzlich maschinell auswertbar, damit Fragen zur Codestruktur direkt dort beantwortet werden können, statt den kompletten Quelltext zu durchsuchen. Details, Installation und Regenerieren siehe `docs/development.md`.
